...
function NavBar() {
  ...
  const listVariants = {
    ...
    visible: {
      ...
      transition: {
        type: "tween",
        staggerChildren: 0.25, // Animates child items one after another
        ease: "easeIn",
      },
    },
  };
  ...
  return (
    <div className="w-full h-fit py-6   fixed top-0 z-50 bg-white shadow-md">
      ...
      <div
        className={`lg:hidden z-50 w-10/12 md:w-6/12 h-full bg-white/100 flex flex-col  items-start absolute -bottom-32 right-0 transition-all duration-500 transform ${
      ...
      >
        ...
        >
          ...
          <div className="w-full mt-10 flex justify-around  bg-amber-100">
            {navbarIcons.map((item, index) => (
              <Link
                key={index}
                to={item.path}
